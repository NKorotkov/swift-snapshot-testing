...
func perceptuallyCompare(_ old: CIImage, _ new: CIImage, pixelPrecision: Float, perceptualPrecision: Float) -> String? {
  ...
  // However, some virtualized environments do not have a GPU and therefore do not support Metal.
  ...
  if ThresholdImageProcessorKernel.isSupported {
    ...
    do {
      thresholdOutputImage = try deltaOutputImage.applyingThreshold(deltaThreshold)
    } catch {
      return "Newly-taken snapshot's data could not be loaded. \(error)"
    }
    let averagePixel = thresholdOutputImage.applyingAreaAverage().renderSingleValue(using: context)
    actualPixelPrecision = 1 - averagePixel
    if actualPixelPrecision < pixelPrecision {
      maximumDeltaE = deltaOutputImage.applyingAreaMaximum().renderSingleValue(using: context)
    }
  } else {
  ...
  The pixel with the lowest perceptual precision \(minimumPerceptualPrecision) is less than required \(perceptualPrecision)
  """
}
...
extension CIImage {
  ...
  func renderSingleValue(using context: CIContext, format: CIFormat = .Rf) -> Float {
    var value: Float = 0
    context.render(
      self,
      toBitmap: &value,
